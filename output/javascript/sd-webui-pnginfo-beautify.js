(() => {
  ...
  var i = /\r\n|\r(?!\n)|\n/g;
  var l = new RegExp(`(${i.source})`, i.flags);
  function crlf(e3, n2 = "\n") {
  ...
  var isEmpty = (t3) => {
    ...
    } else if ("object" == typeof t3 && 0 === Object.keys(t3).length) return true;
    return false;
  };
  var first = (t3) => t3[0];
  function buildIndexesObject(t3) {
    const e3 = [t3].flat().filter(Boolean);
    return !isEmpty(e3) && {
      values: new Set(e3),
      max: Math.max(...e3),
      count: 0,
  ...
  var SearchResults = class {
    ...
    prepareSearch() {
      for (const t3 of [this.searchSettings.separatorSearch, this.searchSettings.bracketsSearch]) t3.lastIndex = 0;
      Object.assign(this, {
        brackets: [],
        pipe: [],
        currentMentions: [],
    ...
    getMentions(t3, e3) {
      const r3 = [], s = [];
      for (const n2 of this.currentMentions) n2.index >= t3 && n2.index < e3 ? r3.push(n2.mention) : s.push(n2);
    ...
    checkSeparator(t3) {
      const { string: e3 } = this, { check: r3, includePositions: s, mentions: n2 } = this.searchSettings;
      let { 0: i2 = "", index: a = e3.length, searchWithinData: o } = null != t3 ? t3 : {};
      const c = i2.length, h = o ? o.openPosition : this.position;
      let p2 = e3.substring(h, a);
      i2 || (this.isDone = true), p2 = this.trimResultText(p2), i2 = this.trimSeparatorText(i2);
      ...
      if (s && (p2 = {
        ...
        const [t4, e4] = this.getMentions(h, a);
        t4 && (p2.mentions = t4, l2 = e4);
      }
      if (r3 && i2) {
        const t4 = isNaN(this.tempPosition) ? h : this.tempPosition;
      ...
      return l2 && (this.currentMentions = l2), this.position = a + c, [p2, u, true];
    }
    pushToPipe(t3) {
      if (this.indexes) {
        if (!this.indexes.hasIndex()) return;
        this.indexes.isOverMax() && (this.isDone = true);
      }
      this.pipe.push(t3);
    }
    addToPipe(t3) {
      ...
      switch (this.searchSettings.includeSeparatorMode) {
        ...
        case "LEFT":
          ...
          break;
        case "RIGHT":
          !("object" == typeof e3 ? e3.text : e3) && !this.lastSeparator || this.pushToPipe([this.lastSeparator, e3]), this.lastSeparator = r3;
          break;
        case "ONLY":
    ...
    findBrackets() {
      const { searchString: t3, brackets: e3, freeArea: r3, searchSettings: s } = this, { bracketsSearch: n2, separatorSearch: i2, searchWithin: a } = s, o = a ? () => this.pipeIsEmpty : () => ("number" != typeof r3.start || r3.start !== r3.end) && !r3.end;
      for (; o(); ) {
        var c;
        const o2 = n2.exec(t3);
        if (!o2) {
          if (a || isNaN(r3.start)) return false;
          r3.end = t3.length - 1;
          continue;
        }
        const p2 = o2[0], { close: l2, ignoreMode: u, searchLevels: g } = (h = e3)[h.length - 1] || {};
        let f;
        switch ((p2 === l2 ? 1 : u && 4) || (f = s.bracketsMap[p2]) && 2 || (null === (c = s.mentions) || void 0 === c ? void 0 : c[p2]) && 3) {
          case 1:
            const t4 = e3.pop();
            a ? (true === g || g.includes(e3.length + 1)) && this.addToPipe(Object.assign(o2, {
              searchWithinData: t4
    ...
    findSeparator(t3) {
      const { searchString: e3, freeArea: r3 } = this, { separatorSearch: s } = this.searchSettings;
      let n2;
      for (; !n2; ) {
        if (t3 = t3 || s.exec(e3)) if (t3.index <= r3.end) {
          const e4 = t3.index >= r3.start && this.addToPipe(t3);
    ...
    getNext() {
      let t3;
      for (; this.pipeIsEmpty && !this.isDone; ) this.findBrackets() ? this.searchSettings.searchWithin || (t3 = this.findSeparator(t3)) : this.isDone = true;
    ...
    getRest() {
      const t3 = [];
      let e3;
      for (; null !== (e3 = this.getNext()); ) t3.push(e3);
      return t3;
    }
    *[Symbol.iterator]() {
      this.prepareSearch();
      ...
      let e3;
      do {
        e3 = t3.getNext(), null !== e3 && (yield e3);
      } while (null !== e3);
    }
  };
  function split(t3, e3) {
    const r3 = this.merge(e3);
    let s = new SearchResults(t3, r3);
    return "number" == typeof r3.indexes ? s.getNext() : r3.returnIterator ? s : s.getAll();
  }
  function createSplitFunction(t3) {
    const e3 = split.bind(t3);
  ...
  function getSplitSmartlyArgs(t3, e3) {
    if (null == t3 || !t3.length) throw new RangeError("empty arguments");
    if (3 === t3.length) {
      if (!e3) return t3;
    } else if (1 === t3.length) {
      const e4 = first(t3);
      "string" == typeof e4 ? t3.push(",", {}) : Array.isArray(e4) ? (t3.unshift(null), t3.push({})) : "object" == typeof e4 && t3.unshift(null, ",");
    } else if (2 === t3.length) "string" == typeof t3[0] && t3[1] instanceof RegExp || "string" == typeof t3[1] || Array.isArray(t3[1]) ? t3.push({}) : t3.unshift(null);
  ...
  function prepareSearch(t3, e3) {
    const s = {
      ...
      separators: t3,
      init() {
        return function initSettings(t4) {
          if (Array.isArray(t4.mentions) || "string" == typeof t4.mentions) {
            const e4 = [t4.mentions].flat().filter(Boolean).reduce((e5, r3) => (e5[t4.ignoreCase ? r3.toUpperCase() : r3] = r3, e5), {});
      ...
      merge(t4) {
        return function mergeSettings(t5, e4) {
          return e4 ? (e4 = {
            ...t5,
            ...e4
      ...
      arrayToPattern: (t4) => function arrayToPattern(t5) {
        var e4;
        return null !== (e4 = r) && void 0 !== e4 || (r = new Set(".{}[]^()+*?\\/$|".split(""))), t5.map((t6) => t6 instanceof RegExp ? t6.source : t6.split("").map((t7) => r.has(t7) ? "\\" + t7 : t7).join("")).join("|");
      }(t4),
      createRegExp: (t4) => RegExp(t4, "g"),
      createBracketsMap() {
        return function createBracketsMap(t4) {
          let e4 = t4.brackets = function normalizeBrackets(t5, e5) {
            var r3;
            return true === t5 ? t5 = e5.slice() : "object" != typeof t5 || Array.isArray(t5) ? "string" == typeof t5 && (t5 = t5.split(",").map((t6) => {
              let e6 = t6.trim().split(" ");
              if (2 !== e6.length) {
                if (2 !== first(e6).length) throw new TypeError("open and close parts of brackets should be separated by space symbol");
                e6 = first(e6).split("");
              }
              return e6;
            })) : t5 = Object.entries(t5), null !== (r3 = t5) && void 0 !== r3 ? r3 : [];
          }(t4.brackets, t4.defaultBrackets);
          return e4 = function handleBracketsMapOptions(t5, e5) {
            return e5.ignoreInsideQuotes && t5.unshift(["'", , , true], ['"', , , true]), t5;
          }(e4, t4), t4.bracketsMap = function buildBracketsMap(t5, e5) {
            return t5.reduce((t6, [r3, s2, ...n2]) => {
              1 !== n2.length || e5 || n2.unshift(void 0);
              let [i2 = e5 && 1, a] = n2;
              return "number" == typeof i2 && (i2 = [i2]), t6[r3] = {
                open: r3,
                ignoreMode: a,
                searchLevels: i2,
                close: s2 || r3
      ...
      createBracketsSearch() {
        return function createBracketsSearch(t4) {
          const e4 = Object.entries(t4.bracketsMap).flatMap(([, { close: t5, open: e5 }]) => t5 !== e5 ? [e5, t5] : e5).concat(Object.keys(t4.mentions || {})).filter(Boolean), r3 = t4.arrayToPattern(e4);
          return t4.bracketsSearch = t4.createRegExp(r3), t4;
        }(this);
      },
      createSeparatorsSearch() {
        return function createSeparatorsSearch(t4) {
          const { separators: e4 } = t4;
          if ("string" == typeof e4 || Array.isArray(e4)) {
            const r3 = t4.arrayToPattern([e4].flat().filter(Boolean));
  ...
  function splitSmartly(...t3) {
    let [e3, r3, s] = getSplitSmartlyArgs(t3);
    const { splitFn: n2 } = _splitSmartlyCore(r3, s);
    return null !== e3 ? n2(e3) : n2;
  }
  ...
  function extractPromptAndInfoFromRaw(e3) {
    const n2 = _isRawVersionPlus(e3 = _normalizeInputRaw(e3));
    let t3 = _splitRawToLines(e3), r3 = "", o = "", i2 = "";
    const a = t3.slice();
    if (t3.length) {
      if (n2) {
        var s, p2;
        ...
        let e4 = t3.pop();
        if (e4.startsWith("Steps: ") && (i2 = e4, e4 = void 0), null !== (s = e4) && void 0 !== s || (e4 = t3.pop()), e4.startsWith("Negative prompt: ") && (o = e4.slice(17), e4 = void 0), null !== (p2 = e4) && void 0 !== p2 || (e4 = t3.pop()), r3 = e4, t3.length) throw new TypeError();
      } else {
  ...
  function handleInfoEntries(e3, n2) {
    const t3 = null == n2 ? void 0 : n2.cast_to_snake, r3 = /^0\d/;
    return e3.map(([e4, n3]) => {
      const o = parseFloat(n3), i2 = r3.test(n3) || isNaN(o) || n3 - o != 0;
      return t3 && (e4 = function keyToSnakeStyle1(e5) {
  ...
  function parseFromRawInfo(e3, n2) {
    let t3 = [];
    if (null != n2 && n2.isIncludePrompts) {
      const { prompt: n3, negative_prompt: r3, infoline: o } = extractPromptAndInfoFromRaw(e3);
  ...
  function parseTheme(source) {
    ...
    for (let i2 = 0, len = settings.length; i2 < len; i2++) {
      ...
      for (let j = 0, lenJ = scopes.length; j < lenJ; j++) {
        ...
        result[resultLen++] = new ParsedThemeRule(scope, parentScopes, i2, fontStyle, foreground, background);
      }
    }
    return result;
  }
      ...
      if (a.scopeDepth === b.scopeDepth) {
        ...
        if (aParentScopesLen === bParentScopesLen) {
          for (let i2 = 0; i2 < aParentScopesLen; i2++) {
            const aLen = aParentScopes[i2].length;
            const bLen = bParentScopes[i2].length;
            if (aLen !== bLen) {
              return bLen - aLen;
            }
          }
  ...
  function _tokenizeString(grammar, lineText, isFirstLine, linePos, stack, lineTokens, checkWhileConditions, timeLimit) {
    ...
    function scanNext() {
      const r3 = matchRuleOrInjections(grammar, lineText, isFirstLine, linePos, stack, anchorPosition);
      if (!r3) {
  ...
  var Grammar = class {
    ...
    _tokenize(lineText, prevState, emitBinaryTokens, timeLimit) {
      ...
      const r3 = _tokenizeString(this, onigLineText, isFirstLine, 0, prevState, lineTokens, true, timeLimit);
  ...
  function parseSequence(sequence) {
    ...
    for (let i2 = 0; i2 < sequence.length; i2++) {
      ...
      if (codeInt === 0) {
        ...
        const [offset, color] = parseColor(sequence, i2);
        if (color) {
          commands.push({
            type: "setForegroundColor",
            value: color
  ...
  function explainThemeScopes(theme, scopes) {
    ...
    for (let i2 = 0, len = scopes.length; i2 < len; i2++) {
      const parentScopes = scopes.slice(0, i2);
  ...
  function explainThemeScope(theme, scope, parentScopes) {
    ...
    for (let i2 = 0, len = theme.settings.length; i2 < len; i2++) {
      const setting = theme.settings[i2];
      let selectors;
  ...
  function codeToTokensWithThemes(internal, code, options) {
    ...
    const tokens = syncThemesTokenization(...themes.map((t3) => codeToTokensBase(internal, code, {
      ...options,
      theme: t3.theme
    })));
    const mergedTokens = tokens[0].map((line, lineIdx) => line.map((_token, tokenIdx) => {
      ...
      tokens.forEach((t3, themeIdx) => {
        const { content: _, explanation: __, offset: ___, ...styles } = t3[lineIdx][tokenIdx];
  ...
  function syncThemesTokenization(...themes) {
    ...
    for (let i2 = 0; i2 < themes[0].length; i2++) {
      const lines = themes.map((t3) => t3[i2]);
      const outLines = outThemes.map(() => []);
      outThemes.forEach((t3, i3) => t3.push(outLines[i3]));
      const indexes = lines.map(() => 0);
      const current = lines.map((l2) => l2[0]);
      while (current.every((t3) => t3)) {
        const minLength = Math.min(...current.map((t3) => t3.content.length));
        for (let n2 = 0; n2 < count; n2++) {
          const token = current[n2];
          if (token.content.length === minLength) {
            outLines[n2].push(token);
            indexes[n2] += 1;
            current[n2] = lines[n2][indexes[n2]];
          } else {
            outLines[n2].push({
              ...token,
  ...
  function codeToTokens(internal, code, options) {
    ...
    if ("themes" in options) {
      ...
      const themeRegs = themes.map((t3) => internal.getTheme(t3.theme));
      const themesOrder = themes.map((t3) => t3.color);
      tokens = themeTokens.map((line) => line.map((token) => mergeToken(token, themesOrder, cssVariablePrefix, defaultColor)));
      fg = themes.map((t3, idx) => (idx === 0 && defaultColor ? "" : `${cssVariablePrefix + t3.color}:`) + (applyColorReplacements(themeRegs[idx].fg, colorReplacements) || "inherit")).join(";");
      bg = themes.map((t3, idx) => (idx === 0 && defaultColor ? "" : `${cssVariablePrefix + t3.color}-bg:`) + (applyColorReplacements(themeRegs[idx].bg, colorReplacements) || "inherit")).join(";");
      themeName = `shiki-themes ${themeRegs.map((t3) => t3.name).join(" ")}`;
  ...
  function mergeToken(merged, variantsOrder, cssVariablePrefix, defaultColor) {
    ...
    const styles = variantsOrder.map((t3) => getTokenStyleObject(merged.variants[t3]));
    const styleKeys = new Set(styles.flatMap((t3) => Object.keys(t3)));
        ...
        function applyLineSection(line, start, end, decoration) {
          ...
          if (startIndex === -1 || endIndex === -1) {
            for (let i2 = 0; i2 < lineEl.children.length; i2++) {
              text2 += stringify2(lineEl.children[i2]);
...
\f\r "&'/<=>`.split(""), "\0	\n\f\r \"&'/<=>`".split("")]
  ...
  var Registry2 = class extends Registry$1 {
    ...
    constructor(_resolver, _themes, _langs, _alias = {}) {
      ...
      this._langs = _langs;
      this._alias = _alias;
      _themes.forEach((t3) => this.loadTheme(t3));
      _langs.forEach((l2) => this.loadLanguage(l2));
    }
    getTheme(theme) {
    ...
    async loadLanguage(lang2) {
      if (this.getGrammar(lang2.name))
        return;
      const embeddedLazilyBy = new Set(Object.values(this._langMap).filter((i2) => i2.embeddedLangsLazy?.includes(lang2.name)));
      ...
      if (embeddedLazilyBy.size) {
        for (const e3 of embeddedLazilyBy) {
          delete this._resolvedGrammars[e3.name];
          this._loadedLanguagesCache = null;
          this._syncRegistry?._injectionGrammars?.delete(e3.scopeName);
          this._syncRegistry?._grammars?.delete(e3.scopeName);
          await this.loadLanguage(this._langMap[e3.name]);
        }
      }
    }
    async init() {
      this._themes.map((t3) => this.loadTheme(t3));
    ...
    async loadLanguages(langs) {
      ...
      if (missingLangs.length) {
        const dependents = langsGraphArray.filter(([_, lang2]) => lang2 && lang2.embeddedLangs?.some((l2) => missingLangs.map(([name]) => name).includes(l2))).filter((lang2) => !missingLangs.includes(lang2));
  ...
  function addRow(key2, value, full) {
    let sx = full ? "_full" : "";
    let html2 = `<div class="shiki_infotext_row">`;
    html2 += `<div class="shiki_infotext_label_div shiki_infotext_label${sx}">${key2}</div>`;
    html2 += `<div class="shiki_infotext_value_div shiki_infotext_value${sx} bilingual__trans_ignore_deep">${value}</div>`;
    html2 += `</div>`;
    return html2;
  }
  function renderLayout({
    ...
    html2 += addRow("Positive Prompt", prompt2, true);
    html2 += addRow("Negative Prompt", negative_prompt, true);
    Object.entries(config).forEach(([key2, value]) => {
      if (typeof value === "string") {
        value = value.replace(/[&<>"']/g, (c) => `&#${c.charCodeAt(0)};`);
      }
      html2 += addRow(key2, value);
    });
    html2 += `</div>`;
    return html2;
  }
  ...
  // src/const.ts
  var EXTENSION_NAME = "sd-webui-pnginfo-beautify";
  var tabs = [
    "#html_info_txt2img",
    "#html_info_img2img",
    "#tab_pnginfo .prose.html-log",
    "#html_info_extras"
  ];

  // src/render.ts
  async function renderInfo(parentId, isElem) {
    const app = gradioApp();
    let elem;
    if (isElem && typeof parentId !== "string") {
      elem = parentId;
      parentId = elem.parentNode;
    }
    if (typeof parentId === "string") {
      parentId = app.querySelector(parentId);
    }
    ...
    if (infotext?.length) {
      ...
      prompt2 = await syntaxHighlighter(prompt2);
    ...
    if (!target) {
      elem.insertAdjacentHTML("afterend", `<div class="prose gradio-html shiki_infotext_root">${html2}</div>`);
      target = parentId.querySelector(`.shiki_infotext_root`);
    } else {
...
`;
  ...
  onUiLoaded(async () => {
    initStyle();
    const app = gradioApp();
    let observer = new MutationObserver(async (mutationsList, observer2) => {
      ...
      for (let mutation of mutationsList) {
        ...
        await renderInfo(mutation.target, true);
      }
    });
    let temp = [];
